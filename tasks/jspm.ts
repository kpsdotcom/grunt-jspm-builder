export = function(grunt: IGrunt) {
    ...
    const traceModulesFromFiles = require("./traceModulesFromFiles");
    /*function () {
    ...
    const bundle = (expressionOrTree, dest, options) => {
        return builder[options.sfx ? "buildStatic" : "bundle"](expressionOrTree, dest, options).then(bundle => {
    ...
    grunt.registerMultiTask("jspm", "Bundle JSPM", (self: grunt.task.IMultiTask<any>) => {
        const options: any = self.options({
            sfx: true,
            mangle: true,
            minify: true
        });
        ...
        // Simple build - Use jspm
            ...
            config.load().then(() => {
                self.files.forEach(file => {
                    ...
                    thenables.push(builder[bundle](moduleExpression, file.dest, options).then((bundle) => {
                        if (options.inject) {
                            if (!config.loader.bundles) {
                                config.loader.bundles = {};
                            }
                ...
                Promise.all(thenables).then((bundles) => {
                    bundles.forEach((bundle) => {

                    });
                }, grunt.fail.warn).then(config.save).then(done);
            });
        }
    });
};
